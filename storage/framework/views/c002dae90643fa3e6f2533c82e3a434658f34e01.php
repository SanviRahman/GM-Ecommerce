    ...
    <script>
        $(document).ready(function(){
            ...
            var table = $("#table").DataTable({
                ajax: "<?php echo e(url('admin/slider/create')); ?>",
                pageLength: 50,
                ordering: false,
                columnDefs: [
                    {
            ...
            $(document).on("click", ".btn-edit", function () {
                ...
                $.ajax({
                    url: "<?php echo e(url('admin/slider')); ?>/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
                    ...
                    success: function (data) {
                        ...
                        $('#name').val(data['name']);
                        $('#link').val(data['link']);
                        var html = ('<div class="image-holder image-"><img src="<?php echo e(url("/public/product/thumbnail/")); ?>/' +data['image']+ '"><button type="button" class="btn btn-danger btn-xs waves-effect waves-light remove-image float-right"><i class="mdi mdi-close"></i></button><input type="hidden" name="imageID" value="' + data['image'] + '"></div>');
            ...
            $(document).on("click", "#submit", function (e) {
                ...
                if (type === 'Save') {
                    $.ajax({
                        type: "post",
                        url: "<?php echo e(url('admin/slider')); ?>",
                        data: {
                            'name': name.val(),
                            'link': link.val(),
                            'image': imageID,
                ...
                if (type === 'Update') {
                    $.ajax({
                        type: "PUT",
                        url: "<?php echo e(url('admin/slider')); ?>/" + id,
                        data: {
                            'name': name.val(),
                            'link': link.val(),
                            'image': imageID,
            ...
            $(document).on('click', '.btn-status', function () {
                ...
                $.ajax({
                    type: "post",
                    url: "<?php echo e(url('admin/slider/status')); ?>",
                    data: {
                        'status': status,
                        'id': id,
                        '_token': token
                ...
                if(ids.length > 0){
                    Swal.fire({
                        ...
                        if(t.value){
                            $.ajax({
                                type: "get",
                                url: "<?php echo e(url('admin/slider/delete')); ?>",
                                data: {
                                    'ids': ids
                                },
                                contentType: "application/json",
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\public_html\resources\views/admin/slider/index.blade.php ENDPATH**/ ?>