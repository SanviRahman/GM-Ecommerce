                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="orderNote" class="form-control" placeholder="Order Note" id="orderNote" rows="2">{{ $order->note }}</textarea>
                                 </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    @if(App\Ip::where('ip_address',$order->ip_address)->first())
                                    <form id="unblock-ip-form" action="{{ route('admin.ip.unblock') }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <div class="form-group">
                                            <label for="unblock-ip">Unblock IP Address:</label>
                                            <input type="text" name="ip_address" id="unblock-ip" value="{{ $order->ip_address }}" class="form-control" placeholder="Enter IP address to unblock" readonly required>
                                        </div>
                                        <button type="submit" class="btn btn-success">Unblock IP</button>
                                    </form>
                                    @else
                                      <form id="block-ip-form" action="{{ route('admin.ip.block') }}" method="POST">
                                        @csrf
                                        <div class="form-group">
                                            <label for="block-ip">Block IP Address:</label>
                                            <input type="text" name="ip_address" id="block-ip" value="{{ $order->ip_address }}" class="form-control" placeholder="Enter IP address to block" readonly required>
                                        </div>
                        ...
                        <table id="productTable" style="width: 100% !important;" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="productName d-block">{{ $product->productName }}</span>
                                        ...
                                        @endif
                                        <?php  $p = App\Product::with(['colors', 'sizes','options'])->find($product->product_id); ?>
                                        @if ($p->colors->isNotEmpty())
                                            <select class="form-control form-control-sm mt-2 productColor">
                                                <option value="">Select Color</option>
                                                @foreach ($p->colors as $color)
                                                    <option value="{{ $color->colorName }}" {{ $color->colorName == $product->colorName ? 'selected' : '' }}>
                                                        {{ $color->colorName }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        @if ($p->sizes->isNotEmpty())
                                            <select  class="form-control form-control-sm mt-2 productSize">
                                                <option value="">Select Size</option>
                                                @foreach ($p->sizes as $size)
                                                    <option value="{{ $size->sizeName }}" {{ $size->sizeName == $product->sizeName ? 'selected' : '' }}>
                                                        {{ $size->sizeName }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        @if ($p->options->isNotEmpty())
                                            <select  class="form-control form-control-sm mt-2 productOption">
                                                <option value="">Select Option</option>
                                                @foreach ($p->options as $option)
                                                    <option 
                                                    data-price="{{ $option->pivot->price }}" 
                                                    value="{{ $option->optionName }}" {{ $option->optionName == $product->optionName ? 'selected' : '' }}>
                                                        {{ $option->optionName }}
                                                    </option>
                                                @endforeach
                                            </select>
