    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $("#table").DataTable({
                ajax: "{{ url('manager/section/create') }}",
                pageLength: 50,
                ordering: false,
                columns: [
                    { data: "id" },
            ...
            $(document).on("click", ".btn-edit", function() {
                ...
                $.ajax({
                    url: "{{ url('manager/section') }}/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on('click', '.btn-status', function() {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ url('manager/section/status') }}",
                    data: {
                        'status': status,
                        'id': id,
                        '_token': token
            ...
            $(document).on("click", ".btn-delete", function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        console.log("{{ url('manager/section') }}"+id);
                        $.ajax({
                            url: "{{ url('manager/section') }}/" + id,
                            type: 'DELETE',
                            data: {
