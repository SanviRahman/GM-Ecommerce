    ...
    <script>
        $(document).ready(function(){
            ...
            var table = $("#table").DataTable({
                ajax: "{{url('manager/category/create')}}",
                pageLength: 50,
                ordering: false,
                columnDefs: [
                    {
            ...
            $(document).on("click", ".btn-edit", function () {
                ...
                $.ajax({
                    url: "{{url('manager/category')}}/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on('click', '.btn-status', function () {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('manager/category/status')}}",
                    data: {
                        'status': status,
                        'id': id,
                        '_token': token
                ...
                if(ids.length > 0){
                    Swal.fire({
                        ...
                        if(t.value){
                            $.ajax({
                                type: "get",
                                url: "{{url('manager/category/delete')}}",
                                data: {
                                    'ids': ids
                                },
                                contentType: "application/json",
