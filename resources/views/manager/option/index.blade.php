    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $("#table").DataTable({
                ajax: "{{ url('manager/option/create') }}",
                pageLength: 50,
                ordering: false,
                columns: [
                    { data: "id" },
            ...
            $(document).on("click", ".btn-edit", function() {
                ...
                $.ajax({
                    url: "{{ url('manager/option') }}/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("click", ".btn-delete", function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        console.log("{{ url('manager/option') }}"+id);
                        $.ajax({
                            url: "{{ url('manager/option') }}/" + id,
                            type: 'DELETE',
                            data: {
