...
class Report extends Controller
{
    ...
    public function getProduct(Request $request)
    {
        if ($request->input('dashboard') == 'yes') {
            return $this->getProductStatusCounts($request);
        }
        ...
        $orders = DB::table('orders')
            ...
            ->select(
                ...
                'orders.orderDate',
                'order_products.*',
                DB::raw('SUM(quantity) as total_amount')
            )
        ...
        if (!empty($request['startDate'])) {
            ...
            $endDateStr   = !empty($request['endDate']) ? $request['endDate'] : $startDateStr;
            ...
            $start = Carbon::createFromFormat('Y-m-d', $startDateStr)
                    ->startOfDay()
                    ->format('Y-m-d H:i:s');
                
            $end = Carbon::createFromFormat('Y-m-d', $endDateStr)
                    ->endOfDay()
                    ->format('Y-m-d H:i:s');
                    
            $orders->whereBetween("orders.created_at", [$start, $end]);
        ...
        // Courier filter
        if (!empty($request['courierID'])) {
            $orders->where('orders.courier_id', $request['courierID']);
        }
    
        return DataTables::of($orders)->make();
    }
    public function getProductStatusCounts(Request $request)
    {
        $priority = ['Completed', 'Invoiced', 'Delivered', 'Canceled'];
        ...
        usort($statuses, function ($a, $b) use ($priority) {
            ...
            // If not found in priority, push to end
        ...
        foreach ($statuses as $status) {
            $selects[] = DB::raw(
                "SUM(CASE WHEN orders.status = '" . addslashes($status) . "' THEN 1 ELSE 0 END) AS `" . $status . "_count`"
            );
        }
    
       
        $selects[] = DB::raw("SUM(CASE WHEN orders.status != 'Incomplete' THEN 1 ELSE 0 END) AS total_orders");
        
        // foreach ($statuses as $status) {
        //     $selects[] = DB::raw(
        //         "SUM(CASE WHEN orders.status = '" . addslashes($status) . "' THEN order_products.quantity ELSE 0 END) AS `" . $status . "_count`"
        //     );
        // }
    
        // // Total quantity for all statuses except Incomplete
        // $selects[] = DB::raw(
        //     "SUM(CASE WHEN orders.status != 'Incomplete' THEN order_products.quantity ELSE 0 END) AS total_orders"
        // );
        ...
        $query = DB::table('orders')
            ...
            ->select($selects)
            ->groupBy('order_products.product_id', 'products.productName');
    
        // 4. Apply date filter
        if (!empty($request['startDate'])) {
            $start = Carbon::createFromFormat('Y-m-d', $request['startDate'])->startOfDay()->format('Y-m-d H:i:s');
            $end = Carbon::createFromFormat('Y-m-d', $request['endDate'] ?? $request['startDate'])->endOfDay()->format('Y-m-d H:i:s');
