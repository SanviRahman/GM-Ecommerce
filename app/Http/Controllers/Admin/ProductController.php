...
class ProductController extends Controller
{
    ...
    public function productOptionPrice($id)
    {
        $product = Product::with('media', 'categories', 'sizes', 'colors','options')->findOrFail($id);
        
        $options = Option::where('status','active')->get();
        
        
        
        return view('admin.product.option', compact('product','options'));
    }
    public function updateOptionPrice(Request $request, $id)
    {
        // Validate the request data
        $validated = $request->validate([
            'proOption' => 'nullable|array',
            'proOption.*' => 'exists:options,id',  // Validate that each option exists in the options table
            'optionPrices' => 'nullable|array',
            'optionPrices.*' => 'numeric|min:0',  // Ensure prices are numeric and greater than or equal to 0
        ]);
    
        // Find the product by its ID
        $product = Product::findOrFail($id);
    
        // Remove all existing options for the product
        $product->options()->detach();
        
        if($validated){
            // Loop through the selected options (proOption) and their corresponding prices (optionPrices)
            foreach ($validated['proOption'] as $optionId) {
                $price = $validated['optionPrices'][$optionId] ?? 0; // Default price is 0 if no price is given
        
                // Attach the option to the product with the specified price
                $product->options()->attach($optionId, ['price' => $price]);
            }
        }
        
    
        // Return a response indicating success
        return redirect()->route('admin.product.option-price', $id)
                         ->with('success', 'Option prices updated successfully!');
    }
    ...
    public function store(Request $request)
    {
        ...
        $product->sizes()->attach($request->productSize);
        $product->colors()->attach($request->productColor);
    ...
    {
        if ($request->ajax()) {
            $products = Product::with('media', 'categories:id,categoryName')->select('id', 'productName', 'ProductSlug', 'productImage', 'status','productCode','productRegularPrice','productSalePrice')->orderBy('id', 'desc');
            ...
            return DataTables::of($products)
                ...
                ->addColumn('action', function ($product) {
                    return
                    "<a href='".route('admin.product.option-price', $product->id)."' class='action-icon btn-edit'> <i class='fas fa-1x fa-tags'></i></a>
                    <a href='javascript:void(0);' data-id='".$product->id."' class='action-icon btn-edit'> <i class='fas fa-1x fa-edit'></i></a>
    ...
    {
        $product = Product::with('media','categories','sizes','colors')->where('products.id','=',$id)->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->productName = $request->productName;
        $product->productSlug = $this->slug($request->productSlug,$id);
        $product->productCode = $request->productCode;
        $product->isFreeDelivery = $request->isFreeDelivery;
        $product->isStockOut = $request->isStockOut;
        $product->productDetails = $request->productDetails;
        $product->productRegularPrice = $request->productRegularPrice;
        $product->productSalePrice = $request->productSalePrice;
        $product->MetaTitle = $request->MetaTitle;
        ...
        $product->sizes()->sync($request->productSize);
        $product->colors()->sync($request->productColor);
    ...
    {
        if (isset($request['q'])) {
            $options = Option::query()->where('optionName', 'like', '%' . $request['q'] . '%')->get();
        } else {
            $options = Option::all();
        }
   ...
   public function size(Request $request)
    {
        if (isset($request['q'])) {
            $sizes = Size::query()->where('sizeName', 'like', '%' . $request['q'] . '%')->get();
        } else {
            $sizes = Size::all();
        }
    ...
    {
        if (isset($request['q'])) {
            $colors = Color::query()->where('colorName', 'like', '%' . $request['q'] . '%')->get();
        } else {
            $colors = Color::all();
        }
    ...
    protected function slug($slug, $productId = null)
    {
        $originalSlug = $slug;
        $count = 1;
    
        // Check if a product with this slug exists, excluding the current product
        while (Product::where('productSlug', $slug)
