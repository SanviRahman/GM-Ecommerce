...
class OrderController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $orders = $orders->where('orders.deleted_at',null);
        ...
        if ($status == 'Incomplete') {
            $orders = $orders->whereIn('orders.status', ['Incomplete']);
        }
        ...
        return DataTables::of($orders->latest('id'))
            ->addColumn('customerInfo', function ($orders) {
                $highlight = '';
                $customOrder = '';
                // Get duplicate phone numbers for "Processing" orders
                ...
                // // Check if the order's status is "Processing" and if the customer's phone is a duplicate
                if (in_array($orders->customerPhone, $duplicatePhones)) {
                    $highlight = "highlight"; // Highlight row with yellow background
                }
            ...
            ->addColumn('action', function ($orders) {
                if (Auth::user()->role_id < 2) {
                    // <a href='".route('admin.pathao_create',['order_id'=>$orders->id])."'> Pathao</a>
                    return "
                     
                    <a href='javascript:void(0);' data-id='" . $orders->id . "' class='action-icon btn-edit'> <i class='fas fa-1x fa-edit'></i></a>
    ...
    public function import(Request $request){
        ...
        if ($type == 'steadfast') {
            ...
            foreach ($orders as $order) {
                ...
                // Send cURL request for this order
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $base_url . '/create_order');
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                ...
                if (isset($resData['status']) && $resData['status'] == 200) {
                    ...
                    $order->tracking_code  = $resData['consignment']['tracking_code'] ?? null;
                    $order->consignment_id = $resData['consignment']['consignment_id'] ?? null;
                    $order->save();
                }
            }
